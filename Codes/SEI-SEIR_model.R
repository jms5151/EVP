...
K_thr_briere <- function(temp, h0, rain, Rmax, N){
  R0 <- 1
  if((rain < R0) | (rain > Rmax)){
    ...
    c <- 7.86e-05
    max(carrying_capacity_th(temp, 29.0, 0.05, N, h0)*c*rain*(rain-R0)*sqrt(Rmax-rain)*0.268 + 0.001, 1000)
  }
}

K_thr_quadratic <- function(temp, h0, rain, Rmax, N){
  R0 <- 1
  if((rain < R0) | (rain > Rmax)){
    max(0.01*carrying_capacity_th(temp, 29.0, 0.05, N, h0), 1000)
  }
    ...
    c <- -5.99e-03
    max(carrying_capacity_th(temp, 29.0, 0.05, N, h0)*(c*(rain-R0)*(rain-Rmax))*0.045 + 0.001, 1000)
  }
}

K_thr_inverse <- function(temp, h0, rain, Rmax, N){
  if (rain < 1){
    rain <- 1
  } 
  max(carrying_capacity_th(temp,29.0,0.05, N, h0)*(1/rain), 1000)
}
