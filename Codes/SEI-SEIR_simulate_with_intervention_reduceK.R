# SEI-SEIR model simulations with intervention: reduce carrying capacity ---------------
...
# load data 
source("Codes/SEI-SEIR_model_with_intervention_reduceK.R")
...
traitFileName <- "Concatenated_Data/model_simulations/SEI-SEIR_simulations_with_intervention_reduceK.csv"
...
for (l in 1:length(sites)){
  ...
  if (sites[l] == "Msambweni"){
    K_thr_reduceK <- K_thr_inverse_reduceK
  } else if (sites[l] == "Machala"){
    K_thr_reduceK <- K_thr_quadratic_reduceK
  } else {
    K_thr_reduceK <- K_thr_briere_reduceK
  }
  ...
  for (m in 1:length(reductionStrategies)){
    ...
    M0 <- K_thr_reduceK(temp[1], hum[1], mean(rain), Rmax, N, percReduce)
    parameters <- c(EFD, pEA, MDR, K_thr_reduceK, a, pMI, mu_th, PDR, b, timestep=timestep)
    state <- c(M1 = startIC$m1*M0, M2 = startIC$m2*M0, M3 = startIC$m3*M0, S = startIC$s*N, E = startIC$e*N, I = startIC$i*N, R = startIC$r*N)
    out <- ode(y = state, times = times, func = seiseir_model_thr_reduceK, parms = parameters, method="rk4", atol = 1e-14, rtol = 1e-14, hini = timestep)
