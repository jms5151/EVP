...
seiseir_model_th <- function(t, state, parameters) {
  with(as.list(c(state,parameters)), {
    ...
    dS <- -a(temp[t])*b(temp[t])*(M3/(M1+M2+M3+0.001))*S + 23.9*(S/1000)/360 - 5.8*(S/1000)/360 + ie*(S+E+I+R) - ie*S
    dE <- a(temp[t])*b(temp[t])*(M3/(M1+M2+M3+0.001))*S-(1.0/5.9)*E - 5.8*(E/1000)/360 - ie*E
    dI <- (1.0/5.9)*E-(1.0/5.0)*I - 5.8*(I/1000)/360 - ie*I
    dR <- (1.0/5.0)*I - 5.8*(R/1000)/360 - ie*R
    list(c(dM1, dM2, dM3, dS, dE, dI, dR))
...
seiseir_model_thr <- function(t, state, parameters) {
  with(as.list(c(state,parameters)), {
    ...
    dM2 <- (a(temp[t])*pMI(temp[t])*I/(S+E+I+R))*M1-(PDR(temp[t])+mu_th(temp[t], hum[t]))*M2
    dM3 <- PDR(temp[t])*M2-mu_th(temp[t], hum[t])*M3
    dS <- -a(temp[t])*b(temp[t])*(M3/(M1+M2+M3+0.001))*S + 23.9*(S/1000)/360 - 5.8*(S/1000)/360 + ie*(S+E+I+R) - ie*S#0.16*R.lag
    dE <- a(temp[t])*b(temp[t])*(M3/(M1+M2+M3+0.001))*S-(1.0/5.9)*E - 5.8*(E/1000)/360  - ie*E
    dI <- (1.0/5.9)*E-(1.0/5.0)*I - 5.8*(I/1000)/360 - ie*I
    dR <- (1.0/5.0)*I - 5.8*(R/1000)/360 - ie*R # 0.16*R.lag
