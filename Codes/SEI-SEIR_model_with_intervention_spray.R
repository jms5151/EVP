# Temperature, humidity, and rainfall dependent SEI-SEIR model with intervention for reducing mosquitoes
seiseir_model_thr_spray <- function(t, state, parameters) {
  with(as.list(c(state,parameters)), {
    dM1 <- EFD(temp[t])*pEA(temp[t])*MDR(temp[t])*mu_th(temp[t], hum[t], timestep)^(-1)*(M1+M2+M3)*max((1-((M1+M2+M3)/K_thr(temp[t], rain[t], Rmax, (S+E+I+R)*2, timestep))),0)-(a(temp[t])*pMI(temp[t])*(I/(S+E+I+R))+mu_th(temp[t], hum[t], timestep))*M1 - intv(Intervention[t])*M1
    dM2 <- a(temp[t])*pMI(temp[t])*(I/(S+E+I+R))*M1-(PDR(temp[t])+mu_th(temp[t], hum[t], timestep))*M2 - intv(Intervention[t])*M2
    dM3 <- PDR(temp[t])*M2-mu_th(temp[t], hum[t], timestep)*M3 - intv(Intervention[t])*M3
...
# This is a function to reduce the mosquito population
intv <- function(x){x}
