...
seiseir_model_thr <- function(t, state, parameters) {
  with(as.list(c(state,parameters)), {
    dM1 <- (EFD(temp[t])*pEA(temp[t])*MDR(temp[t])*mu_th_sigmoidal(temp[t], hum[t])^(-1))*(M1+M2+M3)*max((1-((M1+M2+M3)/K_trh(temp[t], hum[t], rain[t], Rmax, (S+E+I+R)))),0)-(a(temp[t])*pMI(temp[t])*I/(S+E+I+R)+mu_th_sigmoidal(temp[t], hum[t])*M1)
    dM2 <- (a(temp[t])*pMI(temp[t])*I/(S+E+I+R))*M1-(PDR(temp[t])+mu_th_sigmoidal(temp[t], hum[t]))*M2
    dM3 <- PDR(temp[t])*M2-mu_th_sigmoidal(temp[t], hum[t])*M3
...
carrying_capacity_th <- function(temp, T0, EA, N, h0){
  ...
  alpha <- (EFD(T0)*pEA(T0)*MDR(T0)*mu_th_sigmoidal(T0, h0)^(-1)-mu_th_sigmoidal(T0, h0))/(EFD(T0)*pEA(T0)*MDR(T0)*mu_th_sigmoidal(T0, h0)^(-1))
