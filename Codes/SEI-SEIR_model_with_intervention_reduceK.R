...
seiseir_model_thr_reduceK <- function(t, state, parameters) {
  with(as.list(c(state,parameters)), {
    dM1 <- EFD(temp[t])*pEA(temp[t])*MDR(temp[t])*mu_th(temp[t], hum[t], timestep)^(-1)*(M1+M2+M3)*max((1-((M1+M2+M3)/K_thr_reduceK(temp[t], rain[t], Rmax, (S+E+I+R)*2, timestep, percReduce))),0)-(a(temp[t])*pMI(temp[t])*(I/(S+E+I+R))+mu_th(temp[t], hum[t], timestep))*M1
